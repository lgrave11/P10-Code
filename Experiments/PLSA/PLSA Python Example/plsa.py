...
class Corpus(object):
    ...
    def print_matrix(self, matrix, rownumber, coloumnnumber, filepath):
        print("Writing matrix to file: ")
        f = open(filepath, "w")
        for d in range(coloumnnumber):
            for i in range(rownumber):
                f.write( str(matrix[i][d]) + ", ")
            f.write("\n")
        f.close()
    ...
    def plsa(self, number_of_topics, max_iter):
        ...
        self.print_matrix(term_doc_matrix, number_of_documents, vocabulary_size, "./term_doc_matrix_prior.txt")
        self.print_matrix(self.document_topic_prob, number_of_documents, number_of_topics, "./document_topic_prob_prior.txt")
        self.print_matrix(self.topic_word_prob, number_of_topics, len(self.vocabulary), "./topic_word_prob_prior.txt")
        #self.print_matrix(self.topic_prob, number_of_documents, vocabulary_size, "./topic_prob") #3d matrix
        """  
...
#                assert(sum(self.document_topic_prob[d_index]) != 0)
        ...
        self.print_matrix(term_doc_matrix, number_of_documents, vocabulary_size, "./term_doc_matrix_post.txt")
        self.print_matrix(self.document_topic_prob, number_of_documents, number_of_topics, "./document_topic_prob_post.txt")
        self.print_matrix(self.topic_word_prob, number_of_topics, len(self.vocabulary), "./topic_word_prob_post.txt")
