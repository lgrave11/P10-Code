...
def print_document_topic_distribution(corpus, number_of_topics, topk, filepath):
    ...
    assert(topk-1 <= number_of_topics)
    f = open(filepath, "w")
    ...
    for d in range(D):
        ...
        f.write("Document #" + str(d) + " Name: " + corpus.documents[d].name + ":\n")
...
def main():
    ...
    document_paths = ['./reporttest/']#'./texts/grimm_fairy_tales', './texts/tech_blog_posts']#, './texts/nyt']
    #document_paths = ['./test/']
    ...
    print_topic_word_distribution(corpus, number_of_topics, 3, "./topic-word.txt")
    print_document_topic_distribution(corpus, number_of_topics, number_of_topics, "./document-topic.txt")
