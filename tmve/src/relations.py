...
class relations:
    ...
    def get_topics(self):
        ...
        self.topics.sort(key=lambda x: self.get_overall_score(x), reverse=True)
    ...
    def get_terms(self):
        if self.terms == []:
            ...
            self.terms.sort(key=lambda x: self.get_term_count(x), reverse = True)
