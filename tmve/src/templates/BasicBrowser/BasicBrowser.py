...
def get_doc_display(doc):
        ...
        end_cur = site.find(b'</h1>')

        start_cur = site[end_cur:].find(b'<table class="vertical-navbox nowraplinks"')
        while (start_cur != -1):
            while (start_cur != -1):
                new_end_cur = end_cur + start_cur + site[end_cur + start_cur:].find(b'</table>')
                start_cur = site[end_cur + start_cur + len('<table') : new_end_cur].find(b'<table')
                end_cur = new_end_cur
            start_cur = site[end_cur:].find(b'<table class="vertical-navbox nowraplinks"')

        # cherrypick desired content, up to a certain length
        new_start_cur = site[end_cur:].find(b'<p')
        new_end_cur = site[end_cur:].find(b'</p>')

        pair_key = 0
        pairs = [(b'<p', b'</p>'), (b'<span class="mw-headline"', b'</span>'), (b'<div id="toctitle">', b'</div>'), (b'<ul>',b'</ul>')]

        content = ''
        while (new_start_cur != -1 and new_end_cur != -1 and len(content) < 5000):
            content_addition = str(site[end_cur+new_start_cur:end_cur+new_end_cur+len(pairs[pair_key][1])].decode("utf-8"))
            if pair_key == 1:
...
def get_xml_topic_graph(relations):
    ...
    for topic in topics:
        ...
        term_keys.sort(key=lambda x: terms[x], reverse = True)
...
def get_xml_doc_graph(relations):
    ...
    for doc in docs:
        ...
        topic_keys = list(topics.keys())
        sorted(topic_keys, key=lambda x: topics[x], reverse=True)
        #topic_keys.sort(reverse=True)
        #topic_keys.sort(key=lambda  x, y: -cmp(topics[x], topics[y]))
        #topic_keys.sort(key=lambda  x, y: -cmp(topics[x], topics[y]))
        #topic_keys.sort(lambda x, y: -cmp(topics[x], topics[y]))
...
def get_xml_topic_docs_column(relation, topic):
    ...
    doc_keys.sort(key=lambda x: docs[x], reverse = True)
...
def get_xml_doc_topics_column(relation, doc): #TODO: some of these functions can be collapsed/generalized
    ...
    topic_keys = list(topics.keys())
    topic_keys.sort(key=lambda x: topics[x], reverse = True)
    #topic_keys.sort(lambda x, y: -cmp(topics[x], topics[y])) 
    topic_num = 0
...
def get_xml_doc_docs_column(relation, parent_doc):
    ...
    doc_keys = list(docs.keys())
    doc_keys.sort(key=lambda x: docs[x])
    #doc_keys.sort(lambda x, y: cmp(docs[x], docs[y]))
